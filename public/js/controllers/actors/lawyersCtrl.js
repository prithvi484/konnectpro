...
cpApp.controller('lawyersCtrl', ['$scope', '$state','$http', 'userService', function ($scope, $state,$http, userService) {
	...
	userService.getCategories('lawyer')
	.then(categories=>{
    ...
    $scope.filterRefresh = ()=>{
		...
		userService.getActors('lawyer', keys)
		.then(data=>{
			$scope.data = data
		})
	}
	...
	$scope.links =[
		{
			"goto":"abcd",
			"title":"lawyers in J.P.Nagar"
		},
		{
			"goto":"efgh",
			"title":"lawyers in Jayanagar"
		},
		{
			"goto":"",
			"title":"lawyers in malleshwaram"
		},
		{
			"goto":"",
			"title":"lawyers in Jayanagar"
